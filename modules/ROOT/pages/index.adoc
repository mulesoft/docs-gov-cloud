...
MuleSoft Government Cloud is a secure, Federal Risk and Authorization Management Program (FedRAMP)-moderate certified deployment environment that enables government agencies to use Anypoint Platform in the cloud.
...
|Mule Runtime Engine
|Supports Mule runtime engine versions 4.3, 4.4, 4.6, and 4.9 on the long-term support (LTS) release channel. The Edge release channel is not supported, as it is not FedRAMP-approved. For more information, visit xref:release-notes::mule-runtime/lts-edge-release-cadence.adoc[].
