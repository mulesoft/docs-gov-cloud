...
must be globally unique among every other MuleSoft Government Cloud customer and Mule application. A dedicated load balancer helps 
...
In addition, you can set a CNAME record in your corporate DNS nameserver to mask the `lb-name.lb-gprod-rt.anypointdns.net` 
...
to `myapp.example.com` to `myApp.lb-name.lb-gprod-rt.anypointdns.net`. This hides the complexity of the Government Cloud domain name 
